...
## Phase 3: Chart Generation (~0 seconds)

### Step 12: Generate 8-12 Charts via Quickchart.io URLs

**METHOD: Quickchart.io URLs (no Python, instant rendering)**

Embed charts directly in the Markdown as image URLs. No script to run, no files to generate — charts render the moment the Markdown preview opens.

**URL format:**
```
https://quickchart.io/chart?w=600&h=300&bkg=white&c=CHART_CONFIG
```

- `CHART_CONFIG` is a URL-encoded Chart.js config object
- Use `encodeURIComponent(JSON.stringify(config))` to encode, or build the URL inline
- Keep configs simple — Chart.js v2 syntax works reliably with Quickchart

**Embed in Markdown as:**
```markdown
*Figure 1 – Quarterly Revenue Progression*
![Figure 1 – Quarterly Revenue Progression](https://quickchart.io/chart?w=600&h=300&bkg=white&c={...encoded...})
...
1. **Quarterly Revenue Progression** — Bar chart, last 5 quarters, consensus dots overlay
2. **Quarterly EPS Progression** — Bar chart, last 5 quarters, consensus dots overlay
3. **Quarterly Margin Trend** — Line chart, total/products/services gross margin
4. **Revenue by Segment** — Grouped bar, current quarter vs. prior year quarter
5. **Geographic Revenue** — Grouped bar, current quarter vs. prior year quarter
6. **Beat/Miss Summary** — Side-by-side bars (actual vs. consensus) for Revenue, EPS, GM
7. **Services & Installed Base** — Bar + line combo (services rev + device count)
8. **Estimate Revisions** — Side-by-side bars showing old vs. new FY estimates
9. **NTM P/E Valuation Bands** — Line chart with bull/base/bear multiple lines
10. **Q2 Guidance vs. Street** — Grouped bar

**Chart Style Guidelines:**
- Keep Chart.js configs minimal — fewer datasets render faster and look cleaner
- Use `backgroundColor` arrays for color-coding (e.g., highlight current quarter)
- Always include `title.text` and keep axis labels concise

## Phase 4: Report Creation (2-3 hours)

### Step 13: Create Markdown Report

Use the Write tool to create an 8-12 section Markdown (.md) report.

See [report-structure.md](report-structure.md) for complete section-by-section templates and formatting requirements.

**Key Steps:**
1. Create Section 1 with earnings summary and quick takeaways
2. Add detailed results analysis (Sections 2-3)
3. Include key metrics and guidance (Sections 4-5)
4. Update investment thesis (Sections 6-7)
5. Provide valuation and estimates (Sections 8-10)
6. Add appendix if needed (Sections 11-12)
7. Reference all 8-12 chart PNGs with `![Figure N – Title](charts/figN_name.png)`
8. Add 1-3 summary tables using GFM pipe-table syntax
9. Include complete sources section with markdown hyperlinks
...
**Note**: For earnings updates, a full XLS file is OPTIONAL (not required like in initiation reports). The Markdown (.md) report is the primary deliverable.
...
1. **Markdown file**: `[Company]_Q[X]_[Year]_Earnings_Update.md`
